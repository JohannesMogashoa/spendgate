...
function evalConditions(
	...
	return conditions.every((cond) => {
		...
		switch (op) {
			...
			case "eq":
				if (field === "merchant")
					return String(actual) === String(value).toLowerCase();
				return (actual as number) === numVal;
			default:
				return false;
		}
	...
	for (const rule of active) {
		...
		for (const action of rule.actions) {
			if (action.type === "block") {
				...
				actions.push(
					`Notify via ${action.channel ?? "push"} — "${rule.label}"`,
				);
			}
		}
...
export function ruleSummary(rule: SpendRule): string {
	...
	const condParts = rule.conditions.map((c) => {
		...
		if (c.field === "merchant" && c.op === "contains")
			return `merchant contains "${c.value}"`;
