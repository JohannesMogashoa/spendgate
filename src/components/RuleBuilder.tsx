...
export function RuleBuilder({ onSave }: Props) {
	const [draft, setDraft] = useState<Omit<SpendRule, "id">>(defaultDraft());
	...
	return (
		<div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
			...
			<div className="grid grid-cols-3 gap-3">
				<input
					className="col-span-2 rounded-md border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
...
function ConditionRow({
	...
	const fieldOptions: RuleCondition["field"][] = [
		"amount",
		"merchant",
		"hour",
	];
	...
	return (
		<div className="flex items-center gap-2">
			...
			<select
				...
				value={cond.op}
				onChange={(e) =>
					onChange({ op: e.target.value as RuleCondition["op"] })
				}
			>
				{opOptions.map((op) => (
			...
			<input
				...
				value={cond.value}
				placeholder={
					cond.field === "merchant" ? "e.g. woolworths" : "0"
				}
				onChange={(e) =>
					onChange({
						value:
