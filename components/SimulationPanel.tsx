    ...
    return (
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
            <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-semibold text-slate-800">Simulate Transaction</h2>
            </div>

            {/* Inputs */}
            <div className="grid grid-cols-2 gap-3">
                <label className="space-y-1">
                    <span className="text-xs font-medium text-slate-500">Amount (cents)</span>
                    <input
                        type="number"
                ...
                <label className="col-span-2 space-y-1">
                    <span className="text-xs font-medium text-slate-500">
                        Date &amp; Time{" "}
                        <span className="text-slate-400">(hour: {localHour}:00)</span>
                    </span>
                    <input
                        type="datetime-local"
                        className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            {result && (
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        <span
                            className={`font-semibold text-sm ${
                                result.decision === "blocked" ? "text-red-700" : "text-green-700"
                            }`}
                        >
                    ...
                    {result.actions.length > 0 && (
                        <div>
                            <p className="text-xs font-medium text-slate-600 mb-1">Actions:</p>
                            <ul className="text-xs text-slate-600 space-y-0.5 list-disc list-inside">
                    ...
                    {result.triggeredRules.length === 0 && (
                        <p className="text-xs text-slate-500">No rules matched this transaction.</p>
                    )}
                </div>
            )}
        </div>
